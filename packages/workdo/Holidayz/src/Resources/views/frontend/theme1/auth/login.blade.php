...
<head>
    ...
    <title>{{ _('Login/Register') }} | {{ config('APP_NAME', ucfirst($hotel->name)) }}</title>
                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="login-left">
                            ...
                            {{ Form::open(['route' => ['customer.register', $slug], 'method' => 'post', 'id' => 'customer-login']) }}
                            <div class="create-account">
                                <h4 class="h3">{{ __('Create an Account') }}</h4>
                                <p>{{ __('Please enter your email address to create an account.') }}</p>
                                <div class="error-message-register" style="color: red;"></div>
                                <div class="form-group">
                                    <label for="name" class="form-label">{{ __('Name') }}</label>
                                    <input class="form-control @error('name') is-invalid @enderror" id="name"
                                        type="text" name="name" value="{{ old('name') }}" placeholder="{{__('Enter Your Name')}}" required
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control @error('email') is-invalid @enderror" id="email"
                                        type="email" name="email" value="{{ old('email') }}" placeholder="{{__('Enter Your Email')}}" required
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="password_confirmation" type="password"
                                        data-indicator="password_confirmation"
                                        class="form-control pwstrength @error('password_confirmation') is-invalid @enderror"
                                        name="password_confirmation" required autocomplete="new-password"  placeholder="{{__('Enter Your Confirm Password')}}">
                                ...
                                <button href="#" class="btn d-flex align-items-center login-btn">
                    ...
                    <div class="col-md-7">
                        {{ Form::open(['route' => ['customer.login', $slug], 'method' => 'post', 'id' => 'customer-login']) }}
                        <div class="login-right">
                            ...
                            <div class="create-account create-account-right">
                                <h4 class="h3">{{ __('Already Registered?') }}</h4>
                                <div class="input-form">
                                    <div class="form-group">
                                        <label for="email" class="form-label">{{ __('Email Address :') }}</label>
                                        <input class="form-control @error('email') is-invalid @enderror"
                                            id="email" type="email" name="email" placeholder="{{__('Enter Your Email')}}" required
                                            autocomplete="email" autofocus>
                                        @error('email')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control @error('password') is-invalid @enderror"
                                            id="password" type="password" name="password" placeholder="{{__('Enter Your Password')}}" required
                                ...
                                <a href="{{ route('hotel.customer.password.request',$slug) }}" class="align-items-center justify-content-end mobile-direction-column" style="padding-left: 0px;padding-bottom: 5px;">
                                ...
                                @if (Route::has('customer.password.request'))
                                    <a href="{{ route('customer.password.request') }}"
                                        class="forget-btn text-xs">{{ __('Forgot Your Password?') }}</a>
                                @endif
                            </div>
                            {{ Form::close() }}
